...
class YOLOv5HSVRandomAug(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # for box in bboxes:
        #     cv2.rectangle(results['img'], (int(box.tensor[0][0]), int(box.tensor[0][1])),
        #                   (int(box.tensor[0][2]), int(box.tensor[0][3])), (0, 0, 255))
        # cv2.imwrite('/home/ubuntu/mmyolo/{}'.format(results['img_id'])+'.jpg', results['img'])
        return results
...
@TRANSFORMS.register_module()
class Load_4PasetImages(MMCV_LoadImageFromFile):
    """Adds detection objects to the original image.
    ...
    def __init__(self, class_names=None,
        ...
        for class_name in self.class_names:
            if class_name == 'person':
                self.catID[class_name] = 0
                continue
            if class_name == 'chair':
                self.catID[class_name] = 2
                continue
        ...
        self.pipline = self.Compose([self.RandomRotate(rotate=20, prob=0.5),
                                     ...
                                     # self.HomoGrapy(),
                                     # self.RandomlFlip(),
                                     self.RandomErase(prob=0.5, dsize=0.2)])
    ...
    class RandomRotate(object):
        def __init__(self, rotate=15, prob=0.5):
            self.angle = rotate
            self.prob = prob
    ...
    def Embedding(self, image, icon, gt_bboxes):

        image_height, image_width = image.shape[:2]
        icon_height, icon_width = icon.shape[:2]

        max_size = np.random.uniform(self.ICON_FACTOR[0], self.ICON_FACTOR[1]) * min(image_width, image_height)
        ...
        new_bbox = np.array([np.float32(x1), np.float32(y1), np.float32(icon_width+x1), np.float32(icon_height+y1)])
        ...
        for bbox in gt_bboxes:
            ...
            iou_list.append(iou)
            inner_judge = self.inner_bbox_judge(new_bbox, bbox)
            inner_judge_list.append(inner_judge)
        if len(iou_list) > 0 and (max(iou_list) > 0.1 or max(inner_judge_list) is True):
            return np.array([np.float32(0), np.float32(0), np.float32(0), np.float32(0)])
        else:
    ...
    def inner_bbox_judge(self, Box1, Box2):
        # 贴上去的图比原本的某个gt_box大了一圈，直接覆盖掉了原图的box，因此需要判断一下是否存在该情况
    ...
    def transform(self, results):
        for class_name in self.class_names:
            ...
            if class_name == "person" and self.prob_of_copy[0] < np.random.random():
                continue
            elif class_name == "chair" and self.prob_of_copy[1] < np.random.random():
                continue
            elif class_name in self.class_names[-2:] and self.prob_of_copy[2] < np.random.random():
                continue
            ...
            for _ in range(paste_num):
                ...
                if new_bbox.any() is False:
                    ...
                    results['gt_bboxes'].tensor = torch.cat([results['gt_bboxes'].tensor, torch.from_numpy(new_bbox).reshape(-1,4)], 0)
                    results['gt_bboxes_labels'] = np.append(results['gt_bboxes_labels'], self.catID[class_name])
                    results['gt_ignore_flags'] = np.append(results['gt_ignore_flags'], False)


                # bboxes = results['gt_bboxes']
                # for box in bboxes:
                #     cv2.rectangle(img, (int(box.tensor[0][0]), int(box.tensor[0][1])), (int(box.tensor[0][2]), int(box.tensor[0][3])), (0, 0, 255))
                # cv2.imwrite('/home/ubuntu/mmyolo/{}'.format(image_name), img)
